...
Platform E01 source implementation is locally verified on `codex/independent-platform-foundation`: [first-slice PRD](prds/e01-platform-foundation.md), [authorization/connectivity decision](decisions/006-independent-platform-authorization.md). Hosted project metadata remains disconnected from source until runner pairing exists; live WorkOS/Convex proof requires development configuration. See [handoff and open prerequisite checklist](handoffs/e01-platform-foundation.md) and [evidence](evidence/e01-platform/verification.md). On 2026-09-19 the user deferred provider setup and live acceptance; the existing Codex task remains open for that work. A separate task will build the provider-independent proposal/review foundation with bounded SOL high subagents. That preparatory slice does not satisfy the live Letta/runner/spending gates.

Two independent follow-on tasks are active: [selective Stacki upstream adoption](exec-plans/stacki-selective-upstream-adoption.md) and [WorkOS/Convex platform foundation](exec-plans/independent-platform-foundation.md). See the [coordination handoff](handoffs/parallel-platform-and-upstream.md). Company integration continues separately. Next company slice: [M2-02 — WordPress model and ownership](prds/m2-02-wordpress-model-and-ownership.md), beginning with a concrete company schema proposal and verification of repository/runtime inputs. The [M2 dispatch index](prds/m2.md) records dependencies and repository ownership. Live schema/import/publication/deployment remains unperformed; prepare the reviewable result before requesting any missing authorization. No Agent Hub access is needed for this step.

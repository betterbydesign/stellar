...
The local launcher owns runner and Next service groups through IPC-linked supervisors. A supervisor sends SIGTERM to its own child group on launcher disconnect, SIGINT, SIGTERM or SIGHUP, bounds shutdown, and cleans remaining descendants. The launcher also detects loss of its npm/shell parent. Services cannot intentionally detach from this ownership chain; this is a lifecycle boundary, not an untrusted-code sandbox. The runner retains exclusive data ownership and closes its Astro previews before releasing its lease. Startup checks occupied ports, awaits a runner-ready IPC message, starts Next directly without an extra npm shell, and prints the bootstrap link only after the web app responds. Known startup failures are reported without dumping arbitrary configuration values. A live lease owner is never evicted automatically.

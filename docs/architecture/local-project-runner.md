...
The source engine preserves a leading UTF-8 BOM in target fingerprints and translates PostCSS offsets back to the original source before deriving guarded byte spans. Standard CSS properties compare ASCII case-insensitively for ownership and duplicate detection; custom properties remain case-sensitive. The [selective upstream audit](../research/stacki-selective-upstream-audit.md) records the original fixes and generated preservation tests. Revision, write locking and durable history behavior remain the runner's responsibility.

...
Research date: 2026-09-13. Inspected checkout: `/Users/scottfoster/git/stellar`, commit `800fa5270523e7df3afbcaeee8bdbb3a6fe07b49`, package version `0.1.25`. The local origin is `https://github.com/betterbydesign/stellar.git`; this report evaluates the cloned code at that revision, not an assertion that it matches upstream HEAD.
...
| --- | --- | --- |
| Browser-renderable application | React 18, Vite 6, CodeMirror, xterm, extensive JSX/TSX panels; Electron is the application entry point. [Package](/Users/scottfoster/git/stellar/package.json:1) | Reuse UI selectively. A Vite build produces renderer assets, not a functioning hosted editor. |
| Source editing model | Custom Astro parser/serializer handles elements, components, nested children, text, raw scripts/styles, expressions, loops, conditions, props, and slots. Unrepresentable markup falls back to code. [Parser](/Users/scottfoster/git/stellar/electron/astroParser.js:1) | Valuable core, but not an official compiler AST or a universal lossless parser. Separate filesystem-dependent helpers from parsing and mutation operations. |
| Canvas | Real rendered pages in iframes; zoom/pan and side-by-side desktop/tablet/phone frames. [CanvasView](/Users/scottfoster/git/stellar/src/panels/CanvasView.jsx:4) | Reuse layout and interaction primitives. **The multi-frame canvas is view-only**; editing currently happens in single-device mode. Multi-screen interactive canvas remains work. |
| Selected-element bridge | Electron preload in child frames reports geometry, selection, keyboard state, computed styles, and page height; parent receives messages. [Preload](/Users/scottfoster/git/stellar/electron/preload.js:3) | Move browser-safe instrumentation into a preview-only script. Do not carry the privileged Electron preload into SaaS. |
| Preview source mapping | Generated Astro/Vite config inserts markers in development and patches rendered pages to preserve interaction state. [Generated integration](/Users/scottfoster/git/stellar/electron/main.js:3380) | Extract into a versioned Astro integration; test on each supported Astro version. Preserve clean production output. |
| CSS variables | PostCSS reads variables, selectors, themes, comments, and source positions from stylesheets; GUI and IPC support add, rename, move, and edit. [Reader](/Users/scottfoster/git/stellar/electron/cssVars.js:5), [operations](/Users/scottfoster/git/stellar/electron/preload.js:1852) | Strong foundation for manual tweaks and token editing. Needs semantic token types, permissions, dependency/alias graph, validation, and reviewed migrations for Stellar. |
| Components and props | Component discovery and schema extraction feed visual fields; selected properties and structural edits write source. [Scanner](/Users/scottfoster/git/stellar/electron/main.js:1468), [page read/write](/Users/scottfoster/git/stellar/electron/main.js:2664) | Reuse for developer mode. Curate a stricter block manifest for client mode rather than expose every inferred prop and arbitrary CSS field. |
| Local CMS | Local JSON/source data and Astro collections have GUI operations; content introspection bundles and executes project configuration. [CMS bridge](/Users/scottfoster/git/stellar/electron/preload.js:1841), [introspection](/Users/scottfoster/git/stellar/electron/contentConfig.js:6) | Useful schema/forms and file adapters; not a WordPress or SQL admin backend. Introspection belongs inside the project sandbox. |
| Starter creation | A starter registry currently contains Lumos and runs its scaffolder with `lumos@latest`; native shell handles install and Git. [Starter](/Users/scottfoster/git/stellar/electron/starter.js:19) | Replace floating versions with reviewed blueprint manifests, commit hashes, compatibility metadata, and reproducible dependency locks. |
| File persistence | UI saves are serialized per page/file, with debounce and error handling. [Persistence](/Users/scottfoster/git/stellar/src/pagePersistence.js:1) | Reuse mechanics, but add optimistic concurrency/revision checks. Single-process ordering does not resolve simultaneous user/agent edits. |
...
- Passing tests explicitly accommodate known formatting defects. They do not prove fully lossless editing or arbitrary-project compatibility. [Known expectations](/Users/scottfoster/git/stellar/test/expectations.json:1), [test contract](/Users/scottfoster/git/stellar/test/README.md:25)
...
The project scanner accepts `.astro` and page Markdown/MDX; it does not treat a plain HTML repository as a first-class project. [File discovery](/Users/scottfoster/git/stellar/electron/main.js:936) The parser can edit external HTML fragments when an Astro file imports `.html?raw` and inserts them with `set:html`. That proves useful fragment support, **not a standalone HTML builder**. [HTML chunks](/Users/scottfoster/git/stellar/electron/astroParser.js:2914)
...
Electron exposes `window.avb` methods for native dialogs, filesystem reads/writes, dependency installation, assets, content, Git, terminals, and preview operations. [Bridge](/Users/scottfoster/git/stellar/electron/preload.js:1789) Main-process handlers often accept absolute file paths and operate directly on disk. [Write handler](/Users/scottfoster/git/stellar/electron/main.js:2709) The desktop UI is not evidence of tenant authorization or safe remote execution.
...
- **Project workspace:** isolated checkout and dependency environment, scoped filesystem and Git services, Astro/HTML preview, compilation, tests and agent tools. Execute project configuration here; a child process alone is not a security sandbox. The current introspection worker inherits process environment. [Worker launch](/Users/scottfoster/git/stellar/electron/contentConfig.js:213)
...
Keep editor node identity distinct from parser-local IDs: the parser currently generates incrementing `n...` IDs. [Node IDs](/Users/scottfoster/git/stellar/electron/astroParser.js:27) Feedback and Stellar handoffs need durable artifact IDs plus source revision, file, component/block identity, and fallback locator information so comments survive refactoring.

## Templates, licensing, and external data

Stacki's local LICENSE is MIT and permits modification/distribution subject to preserving its notice. Lumos's official license is also MIT. Preserve source notices and package attribution; a fork that ships a desktop wrapper must change the application's identity and update feed as directed in Stacki's README. This is a source-license observation, not an audit of every transitive dependency, font, photo, paid template, or trademark. [Stacki license](/Users/scottfoster/git/stellar/LICENSE:1), [fork release instructions](/Users/scottfoster/git/stellar/README.md:75), [Lumos license](https://raw.githubusercontent.com/lumosframework/lumos-for-astro/main/LICENSE)

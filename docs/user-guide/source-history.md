...
If Stellar reports that source changed, reload and review the latest revision before making another edit. If a save result is unknown, use **Check save**. When the lookup finds no record and the same session, revision, and history target are still current, **Retry same history request** resends the exact original command and request ID. A missing lookup alone does not mean nothing was saved. After a session or revision change, check again, then reload and review instead of retrying. History remains on disk after closing the project or restarting the runner. A preview failure does not erase an already saved source revision.

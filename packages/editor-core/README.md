...
Each style control exposes `authoredValue` for the local override and optional `fallbackValue` for its parsed fallback declaration. A null authored value with token provenance can therefore still identify the approved fallback token; `resolvedValue` remains the concrete displayed result. A fallback outside the control's approved token/type set is exposed as null. Fallback data is read-only and does not change patch preparation.
...
Source identity and patches retain UTF-8 BOM bytes. PostCSS locations are translated back to the original text before byte offsets are calculated. Standard CSS property ownership uses ASCII case-insensitive matching, preserving the authored spelling when replacing or resetting a declaration and refusing duplicate owners across different spellings. Custom property names remain case-sensitive. Escaped declaration names are unsupported: affected rules remain read-only, and token edits are disabled when token uniqueness cannot be proven across the allowlisted stylesheets. The generated preservation suite covers 32 reproducible source variants across replacement, insertion, reset and token edits, checking exact surrounding bytes, no-op behavior and inverse recovery. Its test-design provenance is recorded in the [upstream audit](../../docs/research/stacki-selective-upstream-audit.md); no upstream parser or serializer was adopted.
